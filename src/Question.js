        ...
        <table>
          <tbody>
            <tr>
              ...
              <td style={{ marginRight: '3%', marginBottom: '0px' }}>
...
const Questions = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      const result = await axios('http://140.117.169.233/api/queryQuestions');
      setQuestions(result.data);
    };
    fetchQuestions();
  }, []);

  return questions.map(({ name, subject, content }, index) => (
    <QA key={index} name={name} subject={subject} content={content} />
  ));
};
...
const Question = () => {
  ...
  const submitInput = async () => {
    const result = await axios.post(
      'http://140.117.169.233/api/creatQuestion',
      inputs,
    );
    const response = result.data;

    if (response !== 'QQ') setResponse(response);
  };
